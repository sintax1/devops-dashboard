(function () {
  ...
  angular.module('DevopsDashboard.pages.dashboard', [
    ...
    'DevopsDashboard.services.userstorage', 'DevopsDashboard.services.sidebar'])

    // Controller
    /** @ngInject */
    .controller('DashboardCtrl', function ($scope, $state, $stateParams, User, UserStorage, SidebarService) {
      var name = ('name' in $stateParams) ? $stateParams.name : 'Dashboard';

      // Populate the Dashboard page
      UserStorage.get({name: name}, function(data) {
      ...
      $scope.widgetFilter = function(widget, type) {
        ...
        if ('authorizedGroups' in widget) {
          widget.authorizedGroups.forEach( function(group) {
            ...
            if (userGroups.includes(groupString)) {
              authorized = true;
            }
          });
        } else {
